...
function providerIcon(provider: string): string {
  ...
  if (provider === 'eventuai') {
    return `<svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
...
export function loginPage(opts: {
  ...
  const providerButtons = providers.map((p) => {
    const isPrimary = p === 'eventuai';
    return `<a href="/auth/start?provider=${encodeURIComponent(p)}"
             class="flex items-center justify-center gap-3 w-full px-4 py-3 rounded-xl border-2 ${isPrimary ? 'border-indigo-500 bg-indigo-600 text-white hover:bg-indigo-700' : 'border-gray-200 text-gray-700 hover:border-indigo-400 hover:bg-indigo-50'} font-semibold transition-all duration-150 group">
            <span class="${isPrimary ? 'text-white' : 'text-gray-600 group-hover:text-indigo-600'} transition-colors">${providerIcon(p)}</span>

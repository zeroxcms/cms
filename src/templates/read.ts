...
function readPageField(field: PageFieldRenderModel): string {
...
function readFieldGrid(fields: PageFieldRenderModel[]): string {
  if (!fields.length) return '';
  return `<div class="grid min-w-0 grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5">${fields.map(readPageField).join('')}</div>`;
}
...
function readFieldSet(
  ...
  const fieldsHtml = readFieldGrid(fields ?? []);
  if (!includeItems) return fieldsHtml;
  const groups = (itemGroups ?? []).map(readItemGroup).join('');
  return fieldsHtml + groups;
}

function readItemGroup(group: ItemGroupRenderModel): string {
  const rows = group.rows.length
    ? group.rows.map(readItemRow).join('')
...
function readItemRow(row: ItemRowRenderModel): string {
  const settings = row.hasSettings
    ? `<div class="space-y-3"><p class="text-xs font-semibold uppercase tracking-wide text-gray-400">Settings</p>${readFieldSet(row.settingsFields, row.itemGroups, false)}</div>`
    : '';
  const content = readFieldSet(row.contentFields, row.itemGroups, true);
...
function readStructured(model: StructuredEditorRenderModel): string {
  const settings = model.hasSettings
    ? `<section class="min-w-0 space-y-3">
        <p class="text-sm font-semibold text-gray-700">Settings</p>
        ${readFieldSet(model.settingsFields, model.itemGroups, false)}
      </section>`
    : '';
    ...
    ? model.blocks
        .map(
          (block) => `<div class="min-w-0 space-y-4 rounded-lg border border-gray-200 p-4">
            ...
            ${
              block.hasSettings
                ? `<div class="space-y-3"><p class="text-xs font-semibold uppercase tracking-wide text-gray-400">Settings</p>${readFieldSet(block.settingsFields, block.itemGroups, false)}</div>`
                : ''
            }
            ${readFieldSet(block.contentFields, block.itemGroups, true)}
          </div>`,
        )
        .join('')
    : '<p class="text-sm text-gray-400">No blocks.</p>';

  return `${settings}
  <div class="min-w-0 space-y-5 rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
    <p class="text-sm font-semibold text-gray-700">Content</p>
    ${readFieldSet(model.contentFields, model.itemGroups, true)}
...
export async function readPage(
  ...
  const metaGrid = [
    metaItem('Slug', textValue(`/${page.slug}`)),
    metaItem('Type', textValue(page.page_type ?? 'default')),
    metaItem('Weight', textValue(String(page.weight))),
    metaItem('Parent', textValue(parentLabel)),
    metaItem('From', textValue(displayDate(page.start))),
    metaItem('Until', textValue(displayDate(page.end))),
    metaItem('Timezone', textValue(page.timezone)),
    metaItem('Editors', `<div class="flex flex-wrap gap-1.5">${editorsHtml}</div>`),
    metaItem('Last modified by', textValue(modifierName ?? '')),
  ].join('');
  ...
  const tagsSection = tagGroups
    ? `<div class="min-w-0 rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
        <p class="mb-3 text-sm font-medium text-gray-700">Tags</p>
  ...
  const versionsSection = versionsHtml
    ? `<div class="min-w-0 rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
        <p class="mb-3 text-sm font-semibold text-gray-700">Versions</p>
  ...
  const versionBanner = isVersionPreview && version
    ? `<div class="mb-4 flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
        ...
        Viewing version from ${escHtml(version.created_at)} (read-only).
        <a href="${escHtml(readBase)}" class="font-semibold underline">View current</a>
      </div>`
    : '';

  const body = `<div class="min-w-0 max-w-4xl px-4 py-5 sm:px-6 sm:py-8 lg:px-8">
    <div class="mb-6 flex items-center gap-3">
      <a href="${escHtml(backHref)}" class="shrink-0 text-gray-400 transition-colors hover:text-gray-600" aria-label="Back">
      ...
      <span class="ml-1 shrink-0 rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-semibold uppercase tracking-wide text-gray-500">Read-only</span>
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-wrap items-center gap-2">
        <span class="text-xs font-semibold uppercase tracking-wide text-gray-400">Language</span>
        ${languageLinks}
      </div>

      ${readStructured(model)}
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
        <a href="${escHtml(editHref)}" class="w-full rounded-lg bg-indigo-600 px-6 py-2 text-center text-sm font-semibold text-white shadow-sm transition-colors hover:bg-indigo-700 sm:w-auto">Edit</a>
        <a href="${escHtml(backHref)}" class="w-full rounded-lg border border-gray-300 bg-white px-6 py-2 text-center text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-50 sm:w-auto">Back</a>
      </div>
    </div>
  </div>`;
